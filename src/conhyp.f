...
C     ****************************************************************
      ...
      END
C       PROGRAM SAMPLE
C       COMPLEX*16 A,B,Z,CONHYP,CHF
C       INTEGER LNCHF,IP
C       DOUBLE PRECISION AR,AI
C *
C       OPEN (5,FILE='DATA',STATUS='OLD')
C       READ (5,*) A
C       WRITE (6,*) ' A= ',A
C       READ (5,*) B
C       WRITE (6,*) ' B= ',B
...
C       WRITE (6,*) ' Z= ',Z
C       READ (5,*) LNCHF
C       WRITE (6,*) ' LNCHF= ',LNCHF
C       READ (5,*) IP
C       WRITE (6,*) ' IP= ',IP
C       CHF = CONHYP(A,B,Z,LNCHF,IP)
C       WRITE (6,300) CHF
C       AR=2.31145634403113D-12
C       AI=-1.96169649634905D-11
C       WRITE (6,301) AR,AI
C   300 FORMAT (1X,' RESULT FROM CONHYP=',1P,2D25.12)
C   301 FORMAT (1X,' EXPECTED RESULT=',3X,1P,2D25.12)
C       STOP
C       END
C (-15.0D0,55.0D0)
C (20.0D0,25.0D0)
